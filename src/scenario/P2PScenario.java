	...
	public P2PScenario(String scenarioFileName) {
		...
		for (AgentID currentAgentId : agents.keySet()) {
			...
			for (int j = 0; j < numberContacts; j++) {
				...
				// check if it's not the current agent and it's not already put in the set
		...
		Iterator<XMLNode> ownedItems = scenario.getRoot().getFirstNode("timeline").getFirstNode("items")
				.getNodeIterator("owned");
		// FIXME: cheat 
