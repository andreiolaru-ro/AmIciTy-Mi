...
public class KCA extends JFrame implements Runnable, UpdateListener
{
	/**
	 * 
	 */
	...
	private final static int		PRINTSTEP		= 1;							// number
																					// of
																					// steps
																					// at
																					// which
	...
	private final static Log.Level	LEVEL			= Log.Level.INFO;				// initial
																					// level
																					// of
																					// logging,
																					// and
	private final static int		LEVELSWITCH		= -1;							// after
																					// this
																					// number
																					// of
																					// steps.
																					...
																					// never
																					// change
	private final static Log.Level	LEVELTO			= Log.Level.INFO;				// switch
																					// to
																					// this
																					// level

//	private final static String		SCENARIO_NAME	= "test/test3r_save.xml";
	private final static String		SCENARIO_NAME	= "test/testSmall2.xml";
//	private final static String		SCENARIO_NAME	= "test/test1.xml";
...
//		WindowLayout layout = new WindowLayout(0, 0, 1280, 1000, 15, 1, 5, true, true); // (1366),
		...
		row = layout.addRow(Type.GLOBAL_FACT_NUMBER_GRAPH, ndata + 1,
		...
		row = layout.addRow(Type.GLOBAL_GOAL_NUMBER_GRAPH, 3, new AbstractGraphViewer.GraphParam(
				null, new AbstractGraphViewer.GraphLink("LG"), new Integer(1)));
	...
	public void createMainWindow(int x, int y, int w, int h)
	{
		...
		final JButton randomize = new JButton("Randomize " + Scenario.getSeed());
		randomize.addActionListener(new ActionListener()
		{
			@Override
			public void actionPerformed(ActionEvent e)
			{
				Scenario.resetRandom();
				randomize.setText("Randomize " + Scenario.getSeed());
			}
		});
		box.add(randomize);
	...
	private void init1()
	{
		...
		nextcommand = 0;
//		XMLParser2 testParser = new XMLParser2("playground/xml/scenario.xml") ;
	...
	void doCommand(Command command)
	{
		if (command.getAction() == Command.Action.INJECT)
		{
			...
			AgentID receiver = cm.inject(command.getLocation(), new Message(null,
					Message.Type.INFORM, command.getFact().toCollection()));
		...
		} else if (command.getAction() == Command.Action.REQUEST)
		{
			...
			cm.inject(command.getLocation(), new Message(null, Message.Type.REQUEST, command
					.getFact().toCollection()));
	...
	public static KCA getKca()
	{
		return kca;
	}

	public static void setKca(KCA kca1)
	{
		KCA.kca = kca1;
