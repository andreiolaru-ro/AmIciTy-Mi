 ...
 * 
 *         has quick updating - considers that objects' compared measures dont't
 *         update by large amounts, so it is better to 'move' the object more to
 *         the left or to the right, rather than re-inserting it.
 * 
 * @param <T>
 */
public class QuickSet<T> extends AbstractList<T> implements SortedSet<T> {
	...
	protected class QuickIterator implements Iterator<T> {
		...
		/**
		 * evaluates if it should be right to insert the element in the set by a
		 * subsequent insertHere operation
		 */
		protected boolean evaluateHere(T element) {
			if (!removable)
				throw new IllegalStateException("bad state while evaluating position for "
						+ element);
			if (!backwards)
		...
		protected int evaluateDir(T element) {
			if (!removable)
				throw new IllegalStateException("bad state while evaluating direction for "
						+ element);
		...
		/**
		 * inserts a new element between the element that was last visited and
		 * the current element
		 */
	...
	protected static class QNode<T> {
		...
		// for interior nodes
		protected QNode(@SuppressWarnings("hiding") QNode<T> prev, T element,
				@SuppressWarnings("hiding") QNode<T> next) {
			this.prev = prev;
			this.next = next;
		...
		// for end nodes
		protected QNode(QuickSet<T> list, @SuppressWarnings("hiding") QNode<T> prev, T element,
				@SuppressWarnings("hiding") QNode<T> next) {
			this.prev = prev;
			this.next = next;
			this.content = element;
	...
	public boolean update(T element) {
		...
		it.resync();
		// find direction
		if (it.evaluateDir(element) < 0) { // must move forward
											// search
			...
			return true;
		} else if (it.evaluateDir(element) > 0) { // must move backward
													// search
