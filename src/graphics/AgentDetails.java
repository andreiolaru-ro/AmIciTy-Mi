...
public class AgentDetails extends AbstractViewer2D {
	...
	protected void draw(Graphics2D g) {
		...
		// drawCell(g, cm.cellAt(i, j), i * w + (i + 1) * padding, j * h + (j +
		// 1) * padding, w, h);
		// }
		// }
	}
	...
	// }
	//
	// protected static void drawCell(Graphics2D gg, Agent cell, int left, int
	...
	// gg.fillRect(left + border, top + border, w + 2 * padding, h + 2 *
	// padding);
	...
	// int cs = (int) Math.floor(Math.sqrt((dw - cp) * (dh - cp) /
	// cell.getCapacity())) - cp;
	// int nc = (dw - cp) / (cs + cp);
	// int nr = (dh - cp) / (cs + cp);
	...
	// Map<Data, Collection<Fact>> facts = new HashMap<Data,
	// Collection<Fact>>();
