...
public class XMLTree
{
	...
	public static class XMLNode
	{
		...
		/**
		 * @param childName a {@link String} name corresponding at the name of the first child you want to get 
		 * @return First child node of name input in parameters, null if it doesn't exist.
		 */
		public XMLNode getFirstNode(String childName){
			return getNode(childName, 1) ; // get first indexOf
		}

		/**
		 * @param childName a {@link String} name corresponding at the name of the child you want to get
		 * @param indexOf index of the child you want to get with this name, should be positive
		 * @return child node of name and index input in parameters. Return null if there is no child of this name or at this index. 
		 */
		public XMLNode getNode(String childName, int indexOf){
			int iterator = 0; // iterate on the node
			int index = 0;
			
			if(indexOf > 0){
				while (iterator < nodes.size())
				{
					if (nodes.get(iterator).name.equals(childName))
						index ++ ;
