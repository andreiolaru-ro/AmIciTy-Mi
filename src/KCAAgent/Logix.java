...
public class Logix {
	public static class FactComparator implements Comparator<Fact> {
		...
		public int compare(Fact f1, Fact f2) {
			...
			if ((f1.getPersistence() < zeroPersistence) && (f2.getPersistence() >= zeroPersistence))
				return 1;
			else if ((f2.getPersistence() < zeroPersistence)
					&& (f1.getPersistence() >= zeroPersistence))
				return -1;
	 ...
	 */
	static double availableBeliefProcessing(double agentPressure, double lowPressure,
			double highPressure) {
		// revise 0% (or, anyway, some default small value) when above high
		// limit
	...
	static Map<Domain, NumericMeasure> normSpecialty(Map<Domain, NumericMeasure> specMap) {
		...
		if (norm > 1)
			for (Entry<Domain, NumericMeasure> component : specMap.entrySet())
				component.setValue(new NumericMeasure(component.getValue().getValue().doubleValue()
						/ norm, MeasureName.SPECIALTY));
	...
	static Specialty merge(Specialty s1, Specialty s2, float data2factsMergeRatio2) {
		...
		for (Domain dom : Domain.values()) {
			...
			result.put(
					dom,
					new NumericMeasure(Math.sqrt((1 - data2factsMergeRatio2) * v1 * v1
							+ data2factsMergeRatio2 * v2 * v2), MeasureName.SPECIALTY));
		}
	...
	static Specialty updateSpecialty(Specialty toUpdate, Collection<Fact> ownedFacts) {
		...
		// return Logix.merge(toUpdate, Logix.merge(dataSpec, factSpec,
		// data2factsMergeRatio), specUpdateRatio);
	...
	// {
	// return (float)(Math.min(1.0, Math.pow(interestFadePerDistanceUnit,
	// distance)));
	// }
	...
	// {
	// f.setPressure(0.0f); // this fact is not pressing (for me or for
	// others);what was pressing was the fact telling somebody else had the data
	// setFactInterest(f, agentSpec);
	// f.setPersistence(1.0f); // as long as we have the data, the fact must
	// live. when data will be discarded the fact willbe discarded as well
	// return f;
	// }
	//
	// static Fact setNewInjectedDataFact(Specialty agentSpec, Fact f, Fact
	// contextInfo)
	// {
	// f.setPressure(contextInfo.getPressure());
	// setFactInterest(f, agentSpec);
	// f.setPersistence(1.0f); // it is our data and, as far as it is not
	// deleted, the fact must stay
	// return f;
	// }
	//
	...
	// // should only be called for inferred facts, not for received facts (as
	// in INFORMs).
	// // no pressure involved. actually, the fact has just been built. (this
	// should only result from a sendData / receiveData)
	// // i.e. after a receiveData, infer the sending agent has the data; after
	// a sendData, infer the receiving agent [now] has the data
	// f.setPressure(0.0f); // this fact is not pressing (for me or for others)
	// setFactInterest(f, agentSpec);
	// f.setPersistence(neighbourDataFactPersistence); // the fact is inferred
	// about a neighbour
	// return f;
	// }
	//
	// static Fact setDistantDataFact(Specialty agentSpec, Fact f, Fact
	// contextInfo)
	// {
	// if(contextInfo == null)
	// return setDistantDataFact(agentSpec, f);
	// // the original fact (last parameter) is built elsewhere; it has context
	// information attached.
	// f.setPressure(contextInfo.getPressure()); // keep pressure unchanged, it
	// will fade afterwards, maybe
	// setFactInterest(f, agentSpec); // f already contains the specialty
	// information
	// f.setPersistence(contextInfo.getPersistence() * persistanceFade); // it
	// may have to be persistent but that also depends of our interest.
	// return f;
	// }
	//
	...
	static Fact setNewFact(Fact newFact, Fact relatedFact) throws Exception {
		newFact.setPressure((relatedFact.getPressure() >= 1) ? relatedFact.getPressure()
				: (relatedFact.getPressure() * (1.0f - primaryPressureFade)));
	...
	static Goal makeInformGoal(Fact f) { // the fact should also contain some
											// context information
		return new Goal(f).setImportance(f.getPressure());
	}
	...
	static Goal makeNormalGetGoal(Fact f) { // the fact should also contain some
											// context information
											// return new
											// Goal(f.getDataRecursive()).setImportance(Math.max(f.getInterest(),
											// f.getPressure()));
		return null;
	}
	...
	static Goal setFreeGoal(int used, int available, Goal g) { // goal keep
																// capacity
																// available
																// for capacity
																// between MT
																...
																// range between
																// 0.0 and 1.0
		float importance = Math.min((used / (float) available - memoryThresh)
				/ (1.0f - memoryThresh), 1.0f);
	...
	static boolean goodToForget(Fact f) {
		if (f.getPersistence() < Logix.zeroPersistence
				&& AbstractScenario.rand().nextDouble() < 0.1)
			return true;
		return false;
	}
