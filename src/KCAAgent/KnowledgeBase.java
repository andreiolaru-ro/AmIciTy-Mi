...
public class KnowledgeBase implements QuickForgetConfirmer<Fact>
{
	...
	private Map<Fact, Integer>	base		= new HashMap<Fact, Integer>();			// this vector contains *all* facts. even facts that are included in other facts
	// the value is for the number of links to this fact (reference count)
	/*private*/ QuickSet<Fact>		owned		= new QuickSet<Fact>(new Logix.FactComparator(specialty)); // this contains all facts, as rooted in this agent
...
//		return (Math.random() > fact.getInterest()) && (Math.random() > fact.getPersistence() * Logix.persistenceFade(pressure));
//		return (Math.random() > fact.getInterest()) && (Math.random() > fact.getPersistence());
